...
3. Спроси: **«Подтверждаешь? Одежда: [результат], головной убор: [результат]»** — дай оператору подтвердить или переопределить стиль одежды (`military` / `preserve` / `civilian`) и головной убор (`cap` / `none`).
