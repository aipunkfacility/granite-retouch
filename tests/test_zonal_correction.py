...
class TestFaceBrightnessSoftLightening:
    ...
    def test_face_brightness_lightening_affects_all_face_skin(self):
        """Phase 1 delta применяется ко всем face_skin пикселям (v6.5 без rolloff)."""
        from retouch.processing.correction.face_correction import check_face_brightness
        ...
        # highlights тоже осветляются — rolloff только в apply_postprocess
        highlight_before = arr[20:40, 20:40].mean()
        highlight_after = result_arr[20:40, 20:40].mean()
        assert highlight_after > highlight_before, (
            f"Highlights должны осветлиться: {highlight_before:.0f} → {highlight_after:.0f}"
        )
        assert factor > 1.0
...
class TestLevelsNoRolloff:
    """Rolloff удалён из apply_levels/face_brightness_correction (v6.5)."""

    def test_face_brightness_pixels_can_exceed_ceiling(self):
        """apply_levels больше не ceiling-ит — пиксели могут превысить white_ceiling."""
        ...
        face_skin_mask = np.full((100, 100), 255, dtype=np.uint8)

        analytics = {"median_brightness": 220}
        machine_cfg = {"white_ceiling": 200, "rolloff_compression": 0.35,
                       "face_brightness_target_min": 180, "face_brightness_target_max": 200}
        ...
        assert result_arr.max() > 200, (
            "face_brightness не должен ceiling-ить: soft rolloff удалён"
        )
