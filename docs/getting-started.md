...
Результат: поле `analyzer_output` в `order.json` заполняется данными о фото — стиль одежды, головной убор, состав кадра, ракурс, направление взгляда, перечень предметов одежды с тональной рекодировкой и деталями.
