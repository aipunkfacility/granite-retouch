...
- **Shadow floor** применяется только к пересечению `subject_mask` и `face_mask` — не затрагивает волосы и одежду. Для полного отключения: `shadow_floor: 0`.
...
- **Shadow floor** применяется только к пересечению `subject_mask` и `face_mask`, как и в laser_standard. Ограничение: 5.
...
- Shadow floor (2) — минимальная яркость в тенях, предотвращает застой иглы на чистом чёрном. В отличие от laser, **применяется ко всей `subject_mask`** (needle floor для всей фигуры)
- Формат экспорта по умолчанию: **BMP 8-bit grayscale**
