...
- **FaceOval overlay** — интерактивная коррекция овала лица (4 drag handles). Авто-определение через профиль ширины маски, ручная корректировка для нестандартных
- **Advanced Mode** — технические параметры скрыты по умолчанию, доступны по чекбоксу Advanced в панели параметров
- **Pin Face Oval** — кнопка-пин для фиксации овала лица. Ручное перемещение овала автоматически включает Pin, блокируя автообновление из автодетекции
- **Просмотр дизеринга** — кнопка для предпросмотра Jarvis/Stucki дизеринга (доступно для всех станков; метод зависит от станка: jarvis для лазеров, stucki для impact). Без Numba — 30-120 сек с подтверждением
