...
def test_create_and_delete_preset(client, tmp_path, monkeypatch):
    ...
    res = client.post(
        ...
        json={
            "name": "test-isolated",
            "config": {"processing": {"laser_standard": {"stone_gamma": 0.90}}},
        },
    )
    assert res.status_code == 200
    data = res.json()
...
def test_preset_roundtrip(client, tmp_path, monkeypatch):
    ...
    config_data = {"processing": {"laser_standard": {"stone_gamma": 0.85}}}
    res = client.post(
        "/api/presets",
    ...
    assert len(found) == 1
    assert found[0]["config"]["processing"]["laser_standard"]["stone_gamma"] == 0.85
