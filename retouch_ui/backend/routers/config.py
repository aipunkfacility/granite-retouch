...
PUT /api/config использует deep_merge с DEFAULTS — не теряем ключи, которых нет в запросе.
Миграции применяются автоматически при сохранении (v0→v1→v2→v3).
Безопасность: _validate_path_containment блокирует path traversal при сохранении.
"""
